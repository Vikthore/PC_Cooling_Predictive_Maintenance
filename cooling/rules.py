...
def suggest_actions(
    ...
    if status == "red" or severity >= yellow_hi:
        out.append(
            "🔴 Critical: Immediate cooling intervention required. Check fans, thermal paste, and airflow."
        )
    elif status == "yellow" or severity >= green_hi:
        out.append(
            "🟡 Warning: System temperature is elevated. Monitor closely and consider cleaning fans or improving airflow."
        )
    ...
    if (
        ...
        out.append(
            "Clean dust filters and heatsink fins; fan isn’t scaling with heat (low RPM per °C)."
        )
